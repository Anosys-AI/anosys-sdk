...
// Simple namespaced logger — respects ANOSYS_LOG_LEVEL (debug|info|warn|error|silent)
const LEVELS = { debug: 0, info: 1, warn: 2, error: 3, silent: 4 };
const currentLevel = LEVELS[process.env.ANOSYS_LOG_LEVEL?.toLowerCase()] ?? LEVELS.warn;

export const log = {
  debug: (...a) => currentLevel <= 0 && console.debug('[anosys:openai]', ...a),
  info:  (...a) => currentLevel <= 1 && console.info('[anosys:openai]', ...a),
  warn:  (...a) => currentLevel <= 2 && console.warn('[anosys:openai]', ...a),
  error: (...a) => currentLevel <= 3 && console.error('[anosys:openai]', ...a),
};
