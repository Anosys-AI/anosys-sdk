...
def transform_record(record: dict, incremental_tokens: Any = None, context_overrides: dict = None) -> dict:
    ...
    hasThinking = any(b.get('type') == 'thinking' for b in message.get('content', []) if isinstance(b, dict)) if isinstance(message.get('content'), list) else False
    use_claude_pixel = os.environ.get('ANOSYS_CLAUDE_PIXEL', 'true').lower() == 'true'
    ...
    if use_claude_pixel:
        ...
        payload = {
            ...
            'cvs4': userPrompt,
            'cvs5': assistantText,
            'cvs6': message.get('stop_reason') or U,
            'cvs7': permissionMode,
            'cvs8': version,
            'cvs9': pModel,
            'cvs10': slug,
            ...
            'cvs13': rawUuid,
            'cvs14': rawMessageId,
            'cvs15': (message.get('id') if isinstance(message, dict) and msg_type == 'assistant' else U),
            'cvs16': userType,
            'cvs17': subtype,
            'cvs18': hookName,
            'cvs19': hookCommand,
            'cvs20': agentId,
            'cvs21': toolUseId or U,
            ...
            'cvs38': (data.get('speed') or record.get('speed') or U),
            'cvs39': (data.get('inferenceGeo') or record.get('inferenceGeo') or U),
            'cvs40': (record.get('customTitle') or record.get('title') or U),
            'cvs41': (record.get('aiTitle') or U),
            'cvs42': (record.get('sessionTag') or U),
            'cvs43': (record.get('agentMap', {}).get('name') if isinstance(record.get('agentMap'), dict) else U),
            'cvs44': (record.get('agentMap', {}).get('color') if isinstance(record.get('agentMap'), dict) else U),
            'cvs45': (json.dumps(record.get('agentSetting'), separators=(',', ':')) if 'agentSetting' in record else U),
            'cvs46': (record.get('prUrl') or U),
            'cvs47': (record.get('attributionSurface') or U),
            'cvs48': (record.get('sessionMode') or U),
            'cvs49': (record.get('worktreeSession', {}).get('id') if isinstance(record.get('worktreeSession'), dict) else U),
            'cvs50': (json.dumps(record.get('stagedNodes'), separators=(',', ':')) if 'stagedNodes' in record else U),
            'cvs51': (record.get('origamiSummary') or U),
            'cvs52': (record.get('logicalParentUuid') or U),
            'cvs53': (record.get('teamName') or U),
            'cvs54': (record.get('stopSequence') or U),
            'cvs55': (record.get('leafUuid') or U),
            'cvs56': (record.get('prRepository') or U),
            'cvs57': (json.dumps(record.get('fileStates'), separators=(',', ':')) if 'fileStates' in record else U),
            'cvs58': (record.get('collapseId') or U),
            'cvs59': (record.get('summaryUuid') or U),
            'cvs60': (record.get('summaryContent') or U),
            'cvs61': (data.get('statusMessage') or U),
            'cvs62': (json.dumps(data.get('hookSpecificOutput'), separators=(',', ':')) if 'hookSpecificOutput' in data else U),
            'cvs63': (record.get('worktreeSession', {}).get('branch') if isinstance(record.get('worktreeSession'), dict) else U),
            'cvs64': (record.get('serviceTier') or U),
            'cvs65': (record.get('firstArchivedUuid') or U),
            'cvs66': (record.get('lastArchivedUuid') or U),
            'cvs67': (record.get('originalBranch') or U),
            'cvs68': (record.get('originalHeadCommit') or U),
            'cvs69': (record.get('tmuxSessionName') or U),
            'cvs70': (record.get('taskDescription') or U),
            'cvs71': (record.get('taskType') or U),
            'cvs72': (str(record.get('iterations')) if 'iterations' in record else U),
            'cvs73': advisorModel,
            'cvs74': upgradeNudge,
            'cvs75': url,
            'cvs76': priority,
            'cvs77': hookLabel,
            'cvs78': stopReasonTop,
            'cvs79': researchStr or U,
            'cvs80': mcpMetaStr or U,
            'cvs81': summarizeMetadataStr or U,
            'cvs82': compactMetadataStr or U,
            'cvs83': microcompactMetadataStr or U,
            'cvs84': causeStr or U,
            'cvs85': apiErrorStr or U,
            'cvs86': errorDetails,
            'cvs87': originStr or U,
            'cvs88': imagePasteIdsStr or U,
            'cvs89': fileAttachmentsStr or U,
            'cvs90': writtenPathsStr or U,
            'cvs91': commandsStr or U,
            'cvs92': (record.get('worktreeSession', {}).get('originalCwd') if isinstance(record.get('worktreeSession'), dict) else U),
            'cvs93': (record.get('worktreeSession', {}).get('worktreePath') if isinstance(record.get('worktreeSession'), dict) else U),
            'cvs94': (record.get('worktreeSession', {}).get('worktreeName') if isinstance(record.get('worktreeSession'), dict) else U),
            'cvs199': cvs199Json,
            'cvs200': 'ClaudeCodeHook',
            'cvn1': to_num(it),
            'cvn2': to_num(ot),
            'cvn3': to_num(totalTokens),
            'cvn4': to_num(cr),
            'cvn5': to_num(cc),
            'cvn6': to_num(durMs),
            'cvn7': costEstimate,
            'cvn8': logIndex,
            'cvn9': to_num(inc.get('input', it)),
            'cvn10': to_num(inc.get('output', ot)),
            'cvn11': to_num(inc.get('total', totalTokens)),
            'cvn12': to_num(inc.get('cost', costEstimate or 0)),
            'cvn13': to_num(data.get('hookCount') or record.get('hookCount') or U),
            'cvn14': to_num(data.get('maxRetries') or record.get('maxRetries') or U),
            'cvn15': to_num(data.get('retryAttempt') or record.get('retryAttempt') or U),
            'cvn16': to_num(data.get('retryInMs') or record.get('retryInMs') or U),
            'cvn17': to_num(inc.get('cache_read', cr)),
            'cvn18': to_num(inc.get('cache_creation', cc)),
            'cvn19': to_num(record.get('timeSavedMs') or data.get('timeSavedMs') or U),
            'cvn20': to_num(record.get('replacedResultsCount') or U),
            'cvn21': to_num(record.get('prNumber') or U),
            'cvn22': to_num(record.get('lastSpawnTokens')),
            'cvn23': to_num(promptCount),
            'cvn24': to_num(promptCountAtLastCommit),
            'cvn25': to_num(permissionPromptCount),
            'cvn26': to_num(permissionPromptCountAtLastCommit),
            'cvn27': to_num(escapeCount),
            'cvn28': to_num(escapeCountAtLastCommit),
            'cvn29': to_num(stu_vals.get('web_search_requests', U)),
            'cvn30': to_num(stu_vals.get('web_fetch_requests', U)),
            'cvn31': to_num(budgetTokens),
            'cvn32': to_num(budgetLimit),
            'cvn33': to_num(budgetNudges),
            'cvn34': to_num(messageCount),
            'cvn35': to_num(ttftMs),
            'cvn36': to_num(otps),
            'cvn37': to_num(hookDurationMs),
            'cvn38': to_num(turnDurationMs),
            'cvn39': to_num(toolDurationMs),
            'cvn40': to_num(classifierDurationMs),
            'cvn41': to_num(toolCount),
            'cvn42': to_num(classifierCount),
            'cvn43': to_num(configWriteCount),
            'cvb1': hasThinking,
            'cvb2': record.get('isApiErrorMessage', False),
            'cvb3': isMeta,
            'cvb4': isSidechain,
            ...
            'cvb8': isAgent,
            'cvb9': record.get('armed'),
            'cvb10': (record.get('worktreeSession', {}).get('hookBased') if isinstance(record.get('worktreeSession'), dict) else U),
            'cvb11': isSynthetic,
